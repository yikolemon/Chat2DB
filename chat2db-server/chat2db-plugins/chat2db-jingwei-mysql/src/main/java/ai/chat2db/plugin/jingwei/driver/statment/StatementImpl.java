package ai.chat2db.plugin.jingwei.driver.statment;
...
public class StatementImpl implements Statement {
    ...
    protected JingWeiConnection connection;

    public StatementImpl(JingWeiConnection connection) {
    ...
    public ResultSet executeQuery(String sql) throws SQLException {
        ...
        // 1️⃣ SELECT DATABASE()
        if (upperCaseSql.startsWith("SELECT DATABASE")) {
            Columns columns = new Columns();
            columns.addColumn(1, "DATABASE()");

            Rows rows = new Rows(columns);
            JSONObject jsonObject = new JSONObject();
            String db = connection.getConInfo().getDb();
            jsonObject.put("DATABASE()", db != null ? db : ""); // 没有默认数据库返回空
            FastjsonRow fastjsonRow = new FastjsonRow(jsonObject, columns);
            rows.addRow(fastjsonRow);

            return new ResultSetImpl(rows);
        }

        // 2️⃣ SELECT @@VERSION_COMMENT
        if (upperCaseSql.startsWith("SELECT @@VERSION_COMMENT")) {
            Columns columns = new Columns();
            columns.addColumn(1, "@@version_comment");

            Rows rows = new Rows(columns);
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("@@version_comment", "JING_WEI_PROXY 0.1.0"); // 模拟版本信息
            FastjsonRow fastjsonRow = new FastjsonRow(jsonObject, columns);
            rows.addRow(fastjsonRow);
        ...
        // 3️⃣ SHOW PLUGINS
        if (upperCaseSql.startsWith("SHOW PLUGINS")) {
            Columns columns = new Columns();
            // 可以根据 MySQL SHOW PLUGINS 默认列模拟，或空列
            columns.addColumn(1, "PLUGIN_NAME");
            columns.addColumn(2, "PLUGIN_STATUS");
            Rows rows = new Rows(columns);
            return new ResultSetImpl(rows); // 空插件列表
        }

        // 4️⃣ SELECT @@GLOBAL.SQL_MODE
        if (upperCaseSql.startsWith("SELECT @@GLOBAL.SQL_MODE")) {
            Columns columns = new Columns();
            columns.addColumn(1, "@@GLOBAL.sql_mode");
            Rows rows = new Rows(columns);
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("@@GLOBAL.sql_mode", ""); // 可以模拟空或者默认模式
            FastjsonRow fastjsonRow = new FastjsonRow(jsonObject, columns);
            rows.addRow(fastjsonRow);
            return new ResultSetImpl(rows);
        }

        if (upperCaseSql.startsWith("SHOW DATABASES")) {
            ConnectionInfo conInfo = connection.getConInfo();
            ...
            for (String catalog : catalogs) {
                JSONObject jsonObject = new JSONObject();
                jsonObject.put("Database", catalog);
                FastjsonRow fastjsonRow = new FastjsonRow(jsonObject, columns);
                rows.addRow(fastjsonRow);
            }
            return new ResultSetImpl(rows);
        }
        if (upperCaseSql.startsWith("SHOW VARIABLES LIKE 'LOWER_CASE_TABLE_NAMES'")) {
            // 定义列
            Columns columns = new Columns();
            columns.addColumn(1, "Variable_name");
            columns.addColumn(2, "Value");

            // 构建结果集
            Rows rows = new Rows(columns);
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("Variable_name", "lower_case_table_names");
            jsonObject.put("Value", "1"); // MySQL 默认值，可根据需要修改
            FastjsonRow fastjsonRow = new FastjsonRow(jsonObject, columns);
            rows.addRow(fastjsonRow);
        ...
        return SqlExecUtil.sqlExec(connection.getConInfo(), sql);
    }
